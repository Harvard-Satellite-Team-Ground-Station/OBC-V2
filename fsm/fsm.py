...
class FSM:
    def __init__(self, dp_obj, logger):
        ...
        self.state_objects = {
            ...
            "comms"     : StateComms(dp_obj, logger),
    ...
    def execute_fsm_step(self):
        ...
        # TODO: need to write out comms and orient payload
            # question for comms: does it need to position itself towards Earth?
            # question for orient: what pins do we read to sense light?

        # Comms → Deploy or Orient
        if self.curr_state_name == "comms" and self.dp_obj.data["data_imu_av_magnitude"] <= 1:
